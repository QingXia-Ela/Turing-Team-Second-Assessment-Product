...
; (function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    showHoverInfo: function (obj) { // hover 时显示当天信息
      ...
      var offset_t = $(obj).offset().top - this.$calendar.offset().top + $(obj).width();
      var offset_l = $(obj).offset().left - this.$calendar.offset().left + $(obj).width() + 8;
      var changeStr = _dateStr.substr(0, 4) + '-' + _dateStr.substr(4, 2) + '-' + _dateStr.substring(6);
      var _week = changingStr(changeStr).getDay();
      var _weekStr = '';

      this.$calendar_today.show(); // block 模式

      this.$calendar_today
        .css({ left: (offset_l + 30), top: offset_t, opacity: 0 })
        .stop()
    ...
    renderDOM: function () { // 渲染DOM
      ...
      this.$calendar_today = $('<div class="calendar-today hidden-sm hidden-xs"></div>');


      var _titleStr = '<span href="#" class="title"></span>' +
        '<span href="javascript:;" id="backToday">T</span>' +
        '<div class="arrow">' +
