...
// 字符转换
function encodeSqlContent(target) {
  for (var index in target) {
    target[index] = decodeURI(target[index]);
  }
}
...
function showLocalData() {
  ...
  for (var i = 0; i < eventData.length; i++) {
    encodeSqlContent(eventData[i])
    if (!eventData[i].eventFinish) eventFinishCount++;
    // <input type="checkbox">
    htmlEventStruct.push('<li class="box_shadow" data-id=' + eventData[i].id + '><span class= "list_date">' + eventData[i].year + '-' + eventData[i].month + '-' + eventData[i].day + '</span><p>' + eventData[i].eventTitle + '</p><i class="glyphicon glyphicon-trash"></i><text style="display: none;">' + eventData[i].eventInfo + '</text></li>')
  }
...
function saveLocalData(data, classChoose) {
  ...
  if (data) {
    ...
    if (classChoose) {
      ...
      $.ajax({
        url: 'http://127.0.0.1/api/eventBooking',
        data: { "eventData": temp },
        dataType: "json",
        type: "POST",
        traditional: true,
      ...
      temp = JSON.stringify(temp);
      $.ajax({
        url: 'http://127.0.0.1/api/moneyBooking',
        data: { "moneyData": temp },
        dataType: "json",
        type: "POST",
        traditional: true,
...
function addShowDetails() {
  ...
  $('.small_list li').find('i').on('click', function (e) {
    ...
    $('#delete_confirm .btn-primary').on('click', function () {
      // 判断是哪个模块的
      let deleteSelect = $(item).parent().parent().attr('id');
      $.get('http://127.0.0.1/delete', { classChoose: deleteSelect, deleteID: $(item).parent().attr('data-id') }, function (data) {
        if (data != "OK") {
          alert('删除失败!')
        }
        getLocalData();
